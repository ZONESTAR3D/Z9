...
### Printing status Menu (Hotend Type set to â€œNon-mixingâ€?
![](./LCD_MENU/2.png)
  >
...
### Printing status Menu (Hotend Type set to â€œMixingâ€?
![](./LCD_MENU/3.png)
  >
...
|     Filament       |Slowly length   |How long filament be load/unload while did â€œSlowly Loadâ€?& â€œlowly Unloadâ€|
|     Filament       |Quickly length  |How long filament be load/unload while did â€œQuickly Load â€?& â€?Quickly Unloadâ€œ|
...
|   Bed Leveling     |Catch Probe Z-offset|Auto Catch the probe Z offset (Display only â€œAuto levleingâ€?is ON)|
|   Bed Leveling     |Adjust the probe Z offset|Auto Catch the probe Z offset (Display only â€œAuto levleingâ€?is ON)|
|   Bed Leveling     |Auto leveling|Do auto leveling measure (Display only â€œAuto levleingâ€?is ON)      |
|                    |                |                                                                |
...
| Preheat PLA      |Nozzle Temp        |Set nozzle temperaturewhen do â€œPreheat PLAâ€?on Prepare menu                        |
|                  |Bed Temp           |Set Bed temperature when do â€œPreheat PLAâ€?on Prepare menu                          |
|                  |Fan Speed          |Set FAN speed when do â€œPreheat PLAâ€?on Prepare menu                                |
|                  |Store Settings     |Store above settings                                                               |  
| Preheat ABS      |Nozzle Temp        |Set nozzle temperaturewhen do â€œPreheat ABSâ€?on Prepare menu                        |
|                  |Bed Temp           |Set Bed temperature when do â€œPreheat ABSâ€?on Prepare menu                          |
|                  |Fan Speed          |Set FAN speed when do â€œPreheat ABSâ€?on Prepare menu                                |
|                  |Store Settings     |Store above settings                                                               |  
...
*Display only when â€œControl>> Configure >>Hotend type: mixingâ€? see â€œControl>>Configure"
...
| Mixer            |Set mixer,refer to â€œControl>> Mixerâ€?Menu (only display while â€œHotend type is Mixingâ€?  |
| Configure        |Tune ON/OFF parts of configurations features, refer to â€œControl>> Configureâ€?Menu       |
