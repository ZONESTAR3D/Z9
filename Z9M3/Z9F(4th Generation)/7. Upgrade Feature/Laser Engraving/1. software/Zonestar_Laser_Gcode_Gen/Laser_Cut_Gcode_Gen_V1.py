...
defaults = {
'header': """;========START========
;Generated with ZONESTAR Laser Cut Gcode generator V1
G90 ;
G28	;Home all
""",
'footer': """
...
class laser_gcode(inkex.Effect):
    ...
    def export_gcode(self,gcode):
        ...
        if self.options.laserphase == True:
            f.write(self.options.lasercontrol + " S0 ;laser power off\n") 
        else:
            f.write(self.options.lasercontrol + " S255 ;laser power off\n") 
            
        #header
        f.write(self.header)
        
        #wait for turn on laser power
        if self.options.add_power_on_laser_message == True: 
				    f.write("M0 turn on laser power and press knob ;Wait for laser turn on \n")
				    
				#Z offset
        f.write("G1 Z" + str(int(self.options.z_offset)) + " F400 ;Move up Z axis\n")
        
        #feedrate
        f.write("G1 F" + self.options.travel_speed + "\n")
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option("", "--add-power-on-laser-message", action="store", type="inkbool", dest="add_power_on_laser_message",default=True,help=" ")    		    
        ...
        self.OptionParser.add_option("", "--power-on-delay", action="store", type="int", dest="powerondelay", default="20", help=" ") 
        self.OptionParser.add_option("", "--power-off-delay", action="store", type="int", dest="poweroffdelay", default="100", help=" ") 
        self.OptionParser.add_option("", "--z-offset", action="store", type="int", dest="z_offset", default="10", help=" ")
        self.OptionParser.add_option("", "--laser-speed", action="store", type="int", dest="laser_speed", default="480", help="Laser speed (mm/min)")
        self.OptionParser.add_option("", "--travel-speed", action="store", type="string", dest="travel_speed", default="1200", help="Travel speed (mm/min)")
    ...
    def check_dir(self):
        ...
        if (os.path.isdir(self.options.directory)):
            ...
            if self.options.unit == "G21 (All units in mm)" : 
                self.header += "G21 ;units in mm\n"
            elif self.options.unit == "G20 (All units in inches)" :
                self.header += "G20 ;units in inches\n"
        else: 
...
################################################################################
    def generate_gcode(self, curve, layer, depth):
        ...
        #    Creating Gcode for curve between s=curve[i-1] and si=curve[i] start at s[0] end at s[4]=si[0]
            ...
            if s[1]    == 'move':
                if self.options.laserphase == True:
                    g += "G1 " + c(si[0]) + "\n" + tool['gcode_before_path0'] + "\n"
                    lg = 'G00'
                else:
                    g += "G1 " + c(si[0]) + "\n" + tool['gcode_before_path1'] + "\n"
                    lg = 'G00'
            elif s[1] == 'end':
            		if self.options.laserphase == True:
										g += tool['gcode_after_path0'] + "\n"
										lg = 'G00'
            		else:
            				g += tool['gcode_after_path1'] + "\n"
            				lg = 'G00'
            elif s[1] == 'line':
                if lg=="G00": g += "G1 " + feed + "\n"   
                g += "G1 " + c(si[0]) + "\n"
...
################################################################################
    def effect(self) :
        ...
        self.tools = {
            ...
            "gcode_before_path0": (self.options.lasercontrol + " S" + str(self.options.laserpower) + "\nG4 P" + str(self.options.powerondelay)),
            "gcode_before_path1": (self.options.lasercontrol + " S" + str(255-self.options.laserpower) + "\nG4 P" + str(self.options.powerondelay)),
            "gcode_after_path0": (self.options.lasercontrol + " S0\n" + "G4 P" + str(self.options.poweroffdelay)+ "\nG1 F" + self.options.travel_speed),
            "gcode_after_path1": (self.options.lasercontrol + " S255\n" + "G4 P" + str(self.options.poweroffdelay)+ "\nG1 F" + self.options.travel_speed),
        }
